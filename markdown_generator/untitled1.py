# -*- coding: utf-8 -*-
"""
Created on Wed Sep  3 14:07:19 2025
...
def main():
    ...
    # ✅ Your personal defaults (edit once, then just Run in Spyder)
    ...
    print(f"[INFO] Will write markdowns to: {md_dir}")
    print(f"[INFO] Images subdir: /{images_subdir}/   Files subdir: /{files_subdir}/")
    if args.dry_run:
    ...
    # Case-insensitive CIF discovery
    cif_paths = list(cif_dir.rglob("*.[cC][iI][fF]"))
    print(f"[INFO] Found {len(cif_paths)} CIFs")
    if not cif_paths:
        print("[WARN] No CIFs found. Check the folder path and file extensions (.cif/.CIF).")
        return

    rows = []
    for idx, cif_path in enumerate(cif_paths, 1):
        print(f"[INFO] ({idx}/{len(cif_paths)}) Parsing {cif_path}")
        try:
        ...
        if args.copy_cifs:
            try:
                ...
                cif_link = cif_rel
                print(f"[OK]  Copied CIF -> {cif_dest}")
            except Exception as e:
                print(f"[WARN] Could not copy CIF {cif_path} -> {cif_dest}: {e}")
                cif_link = None
        else:
            # Optimistic link if you plan to add CIFs later
            cif_link = cif_rel
